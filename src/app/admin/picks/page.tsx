...
import PickManagementClient from "@/components/PickManagementClient";
import { Music, ArrowLeft, LayoutDashboard, ChevronRight } from "lucide-react";
...
export default async function PickManagementPage() {
  ...
  if (!session || !["ADMIN", "STAFF"].includes(session.user.role)) {
    if (!session) {
      redirect("/auth/signin?callbackUrl=/admin/picks");
    }
    return <AccessDenied />;
  }
  ...
  return (
    <div className="min-h-screen bg-[url('/main_bg.png')] bg-repeat p-4 sm:p-8 text-white">
      <div className="mx-auto max-w-7xl">
        <header className="mb-12 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <div className="flex items-center gap-2 text-sm text-gray-400 mb-1">
              ...
              <span className="text-[#39C5BB]">추천 영상 관리</span>
            </div>
            <h1 className="flex items-center gap-3 text-3xl font-bold tracking-tight">
              <Music className="text-purple-400" />
              추천 영상 관리
            </h1>
            <p className="mt-2 text-gray-400">메인 페이지 추천 영상 관리</p>
          </div>
        ...
        <PickManagementClient initialPicks={picks} />
      </div>
    </div>
  );
}

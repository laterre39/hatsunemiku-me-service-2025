...
import SiteManagementClient from "@/components/SiteManagementClient";
import { Globe, ArrowLeft, LayoutDashboard, ChevronRight } from "lucide-react";
...
export default async function SiteManagementPage() {
  ...
  if (!session || !["ADMIN", "STAFF"].includes(session.user.role)) {
    if (!session) {
      redirect("/auth/signin?callbackUrl=/admin/sites");
    }
    return <AccessDenied />;
  }
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 text-white">
      <div className="mx-auto max-w-7xl">
        <header className="mb-12 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <div className="flex items-center gap-2 text-sm text-gray-400 mb-1">
              ...
              <span className="text-[#39C5BB]">사이트 링크 관리</span>
            </div>
            <h1 className="flex items-center gap-3 text-3xl font-bold tracking-tight">
              <Globe className="text-indigo-400" />
              사이트 링크 관리
            </h1>
            <p className="mt-2 text-gray-400">푸터 및 관련 사이트 링크 관리</p>
          </div>
        ...
        <SiteManagementClient initialSites={sites} />
      </div>
    </div>
  );
}

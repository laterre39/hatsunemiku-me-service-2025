  ...
  return (
    <ul className="space-y-4">
      {items.map((item, index) => {
        ...
        return (
          <li key={item.id}>
            <Link href={item.href} target="_blank" rel="noopener noreferrer" className="flex flex-col justify-between h-32 rounded-2xl bg-white/[.03] p-3 border border-white/10 transition-all duration-300 hover:bg-white/10 hover:shadow-lg hover:border-white/20 hover:-translate-y-1">
              {/* Top section: Rank, Image, Info */}
              ...
              <div className="flex items-center justify-end gap-3 text-xs text-gray-300">
                  <div className="flex items-center font-semibold gap-1.5 rounded-full bg-white/10 px-2.5 py-1">
                      <Clock size={14} />
...
export default function MusicPage() {
  ...
  const [rankings, setRankings] = useState<{ youtube: YouTubeItem[]; spotify: SpotifyItem[] }>({ youtube: [], spotify: [] });
    ...
    <main ref={mainRef} className="mx-auto max-w-4xl py-12 px-4 scroll-mt-20">
      ...
      {!loading && totalPages > 1 && (
        <div className="mt-8 flex items-center justify-center gap-2">
          ...
          <span className="font-semibold text-white px-4 py-2 rounded-lg bg-white/5">
